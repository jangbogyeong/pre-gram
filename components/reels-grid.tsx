...
const SortableImage = memo(function SortableImage({
  ...
  const handleRemove = () => {
    ...
    // 시각적 피드백을 위한 짧은 지연
    setTimeout(() => {
      console.log("ReelsGrid: Removing image with ID:", image.id)
      onRemove(image.id)
    }, 200)
  }
...
function ReelsGrid({ images, onReorder, onRemoveImage, readOnly = false }: ReelsGridProps) {
  ...
  console.log(
    `ReelsGrid: User uploaded images: ${userUploadedImages.length}, Existing images: ${existingImages.length}`,
  )
  ...
  const handleDragStart = useCallback(
    (event: DragStartEvent) => {
      ...
      console.log(`ReelsGrid: Started dragging image ${event.active.id}`)
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (!over) {
        console.log("ReelsGrid: Drag ended without a target")
        return
      }

      // 같은 위치로 드래그한 경우 무시
      if (active.id === over.id) {
        console.log("ReelsGrid: Dragged to same position, ignoring")
        return
      }

      console.log(`ReelsGrid: Dragged image ${active.id} over ${over.id}`)
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        console.log(`ReelsGrid: Reordering from index ${oldIndex} to ${newIndex}`)
        ...
        console.log(`ReelsGrid: Reordered images, new order has ${reorderedImages.length} images`)
        console.log(`ReelsGrid: User uploaded images after reorder: ${reorderedUserImages.length}`)

        // 부모 컴포넌트에 알림 - 상태 업데이트를 위해 반드시 호출
        onReorder(reorderedImages)
      } else {
        console.warn(`ReelsGrid: Could not find indices for drag operation. Old: ${oldIndex}, New: ${newIndex}`)
      }
    },
  ...
  const handleRemoveImage = useCallback(
    (imageId: string) => {
      console.log("ReelsGrid: Removing image", imageId)
